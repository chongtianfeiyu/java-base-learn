...
public class CompleteKnapsack {
     ...
     * 求解将哪些物品装入背包可使价值总和最大
     *
     * 这个问题类似于01背包问题，所不同的是每种物品有无限件。
     * 也就是从每种物品的角度考虑，与它相关的策略已非取或不取两种，而且右取0件、取1件、取2件...等很多种
     *
     * 子问题状态:f[i][j]:表示前i件物品放入容量为j的背包得到的最大价值
     * 状态转移方程:f[i][j] = max{f[i - 1][j],f[i - 1][j - k*weight[i]] + k*value[i] | 0 <= k * c[i]<= V}
     */
